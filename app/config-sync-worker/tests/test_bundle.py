...
def test_build_consumer_bundle_shape():
    docs = [
        {"id": "consumer:consumer-a", "doc_type": "consumer_config", "consumer": "consumer-a",
         "allowed_models": ["gpt-5.4", "gpt-5.4-mini"], "tier": "medium",
         "tokens_per_minute": 5000, "token_quota": 200000, "token_quota_period": "Daily",
         "daily_budget": 100, "downgrade_ladder": ["gpt-5.4", "gpt-5.4-mini"]},
    ...
    assert bundle["consumer-a"] == {"allowed_models": "gpt-5.4,gpt-5.4-mini", "tier": "medium", "downgrade_ladder": "gpt-5.4,gpt-5.4-mini"}
    assert bundle["consumer-b"] == {"tier": "large"}


def test_encode_bundle_is_base64_json():
    bundle = {"consumer-a": {"allowed_models": "gpt-5.4-mini"}}
...
def test_build_consumer_bundle_skips_docs_without_consumer():
    docs = [
        {"id": "global", "allowed_models": ["gpt-5.4"]},  # no "consumer" → skipped
        {"consumer": "consumer-a", "allowed_models": ["gpt-5.4-mini"]},
    ]
    bundle = sync.build_consumer_bundle(docs)
...
def test_build_consumer_bundle_preserves_string_allowed_models():
    ...
    docs = [{"consumer": "consumer-a", "allowed_models": "gpt-5.4,gpt-5.4-mini"}]
    bundle = sync.build_consumer_bundle(docs)
    assert bundle["consumer-a"] == {"allowed_models": "gpt-5.4,gpt-5.4-mini"}
...
def test_bundle_includes_downgrade_ladder_csv():
    ...
    bundle = sync.build_consumer_bundle(docs)
    assert bundle["smoke"]["downgrade_ladder"] == "gpt-5.4,gpt-5.4-mini,grok-4.3"
