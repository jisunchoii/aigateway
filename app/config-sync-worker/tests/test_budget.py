...
PRICING = {
    "gpt-5.6-sol": {"prompt": 0.0025, "completion": 0.015},
    "FW-GLM-5.2": {"prompt": 0.00075, "completion": 0.0045},
...
def test_cost_for_consumer_sums_prompt_and_completion():
    ...
    usage = {"gpt-5.6-sol": {"prompt": 10000, "completion": 2000}}
    assert budget.cost_for(usage, PRICING) == 0.055


def test_cost_for_multi_model():
    usage = {"gpt-5.6-sol": {"prompt": 1000, "completion": 0},        # 0.0025
...
def test_evaluate_sets_active_downgrade_usd():
    docs = [{"consumer": "smoke", "daily_budget_usd": 1.0,
             "downgrade_ladder": ["gpt-5.6-sol", "FW-GLM-5.2"]}]
    # cost 0.25 (100k prompt) + 0.6 (40k completion) = 0.85 -> 85% -> level 1
    usage = {"smoke": {"gpt-5.6-sol": {"prompt": 100000, "completion": 40000}}}
...
def test_evaluate_skips_no_usd_budget():
    ...
    docs = [{"consumer": "legacy", "daily_budget": 1000,
             "downgrade_ladder": ["gpt-5.6-sol", "FW-GLM-5.2"]}]
    usage = {"legacy": {"gpt-5.6-sol": {"prompt": 9_999_999, "completion": 0}}}
    out = budget.evaluate_downgrades(docs, usage, PRICING)
...
def test_evaluate_skips_no_ladder():
    ...
    out = budget.evaluate_downgrades(docs, {"noladder": {"gpt-5.6-sol": {"prompt": 9_999_999}}}, PRICING)
...
def test_evaluate_clears_when_back_under_usd():
    ...
    usage = {"smoke": {"gpt-5.6-sol": {"prompt": 1000, "completion": 0}}}  # 0.0025 -> 0% -> cleared
    out = budget.evaluate_downgrades(docs, usage, PRICING)
