...
현재 운영 검증된 기본 방식은 **APIM subscription key**입니다. 같은 key라도 클라이언트 SDK가 보낼 수 있는 헤더 이름이 달라 APIM 경로를 분리합니다.

| 클라이언트 | APIM 경로 | key 전달 방식 |
|---|---|---|
| GitHub Copilot CLI (Azure provider) | `/openai` | `api-key: <APIM subscription key>` |
| VS Code BYOK custom endpoint | `/vscode/models` | `Ocp-Apim-Subscription-Key: <APIM subscription key>` |
| 직접 Foundry/OpenAI-compatible 호출 | `/foundry` | `Ocp-Apim-Subscription-Key: <APIM subscription key>` |
