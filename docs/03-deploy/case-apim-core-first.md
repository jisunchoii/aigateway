...
| Admin UI 공개 여부 | 나중에 외부 브라우저에서 열 계획이면 첫 apply부터 `admin_ui_public=true` |
| 기존 계정 재사용  | project management enabled, local auth disabled, public access disabled 완료 |
| 이미지 변수     | 첫 번째 apply에서는 네 이미지 변수 모두 `""` |

{% hint style="warning" %}
`apim_public`, `admin_ui_public`과 모델 backend 설정은 첫 apply 전에 확정하세요. 나중에 바꾸면 APIM 또는 Container Apps 환경, Private Endpoint, RBAC, VNet 구성을 다시 적용해야 합니다.
{% endhint %}
...
apim_public     = true
admin_ui_public = false # 이후 public Admin UI가 필요하면 첫 apply부터 true
