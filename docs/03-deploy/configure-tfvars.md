---
description: 운영자·DevOps 엔지니어를 위한 페이지 · 선행: 상태 백엔드 부트스트랩
---
...
아래 변수들을 우선적으로 설정하세요. 전체 변수 목록과 기본값은 [레퍼런스 — 변수 전체 목록](../10-reference/variables.md)을 참고하세요.
...
기존 AIServices 계정을 재사용한다면 아래 변수를 추가로 설정하세요. 상세 절차는 [기존 Foundry 재사용](../04-reuse-foundry/overview.md) 챕터를 참고하세요.
...
변수 설정이 완료되면 [첫 번째 apply](first-apply.md)로 이동하세요.
