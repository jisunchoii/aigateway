...
{% hint style="warning" %}
`apim_public`과 모델 deployment 설정은 첫 배포 전에 확정하세요. `admin_ui_public`은 `admin_ui_image`를 설정할 때 생성되는 전용 Admin UI 환경에만 적용됩니다.
{% endhint %}
...
# Admin UI 이미지를 배포할 때 생성되는 전용 환경의 공개 여부.
# Codex proxy와 Search MCP의 내부 sidecar 환경에는 영향을 주지 않음.
...
1차 apply에서 만들어진 ACR login server와 Entra 값을 tfvars에 입력합니다. `admin_ui_public`은 이때 생성되는 전용 Admin UI 환경의 노출 방식만 선택합니다.
