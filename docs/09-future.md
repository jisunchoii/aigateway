...
Claude Code는 Anthropic Messages API 형식으로 요청합니다. 현재 gateway의 `/openai/v1`, `/vscode/models` 경로는 OpenAI 호환 형식이므로 Claude Code를 그대로 붙일 수 없습니다.
...
| APIM 경로 | `/openai/v1/responses` — Codex proxy sidecar가 payload를 정규화한 뒤 같은 AIServices 계정의 `codexproj` project `/api/projects/codexproj/openai/v1/responses` 호출 |
| 인증 헤더 | `api-key` |
| 지원 모델 | `gpt-5.6-sol`, `FW-GLM-5.2`, `DeepSeek-V4-Pro`, `grok-4.3` |
| backend 인증 | APIM 관리 ID + RBAC (키 미사용) |
| hosted web search | GPT는 native tool, OSS는 APIM `/mcp/`의 bounded Search MCP 사용 |
