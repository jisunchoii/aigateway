...
| APIM 경로 | `/responses` — Codex proxy sidecar가 payload를 정규화한 뒤 같은 AIServices 계정의 `codexproj` project `/api/projects/codexproj/openai/v1/responses` 호출 |
| 인증 헤더 | `Ocp-Apim-Subscription-Key` (Entra ID 모드도 동일 정책 재사용) |
| 지원 모델 | **`gpt-5.6-sol`만** (native Responses, hosted·function tool 포함). grok·DeepSeek은 Codex 클라이언트 한계로 비권장 → OpenCode 사용 |
