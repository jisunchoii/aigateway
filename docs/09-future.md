...
{% hint style="success" %}
**Codex는 `gpt-5.6-sol`로 지원됩니다.** Codex CLI는 Responses API 전용(`wire_api = "responses"`)이며, `/responses` 경로가 Codex proxy sidecar를 거쳐 같은 AIServices `codexproj` backend의 Responses API로 연결됩니다. grok·DeepSeek 등 비gpt 모델은 native Responses를 지원하더라도 Codex 클라이언트 측 한계로 간헐적 스트리밍 실패가 발생하므로 [OpenCode](07-connect-clients/opencode.md)(Chat Completions)를 사용합니다. [Codex CLI 온보딩](07-connect-clients/codex-cli.md)을 참고하세요.
{% endhint %}
...
Codex는 정식 온보딩 대상으로 이동했습니다. 설정은 아래 문서를 참고하세요.
...
| APIM 경로 | `/responses` — Codex proxy sidecar가 payload를 정규화한 뒤 같은 AIServices project의 `/openai/v1/responses` 호출 |
| 인증 헤더 | `Ocp-Apim-Subscription-Key` (Entra ID 모드도 동일 정책 재사용) |
| 지원 모델 | **gpt 계열만** (native Responses, hosted·function tool 포함). grok·DeepSeek은 Codex 클라이언트 한계로 비권장 → OpenCode 사용 |
| backend 인증 | APIM 관리 ID + RBAC (키 미사용) |
| 미지원 | Fireworks 서빙 등 partner/community OSS(Chat Completions only) — 변환 계층 필요, TBD |
