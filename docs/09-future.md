...
{% hint style="success" %}
**Codex는 gpt 계열 모델로 지원됩니다.** Codex CLI는 Responses API 전용(`wire_api = "responses"`)이며, `/responses` 경로가 AIServices 계정의 native Responses로 직접 프록시합니다(별도 브리지 없음). grok·DeepSeek 등 비gpt 모델은 native Responses를 지원하더라도 Codex 클라이언트 측 한계로 간헐적 스트리밍 실패가 발생하므로 [OpenCode](07-connect-clients/opencode.md)(Chat Completions)를 사용합니다. [Codex CLI 온보딩](07-connect-clients/codex-cli.md)을 참고하세요.
{% endhint %}
...
| APIM 경로 | `/responses` — AIServices 계정의 `/openai/v1/responses`로 직접 프록시 (별도 브리지 없음) |
