...
## terraform plan — 재사용 단언 확인

`terraform apply` 전에 반드시 `plan`을 실행해 재사용이 제대로 설정됐는지 확인한다.
...
재사용 모드가 올바르게 설정됐다면 plan 결과는 반드시 다음 조건을 충족해야 한다.
...
`data.azurerm_cognitive_account`는 provider 버전에 따라 `local_auth_enabled` 속성을 노출하지 않을 수 있다.
이 경우 Terraform precondition으로 사전 점검을 강제할 수 없다.

대신 [prepare-account.md](prepare-account.md)에서 안내한 `az resource show` 명령으로 사전 점검을 직접 수행한다.
...
`disableLocalAuth: true`, `publicNetworkAccess: "Disabled"` 두 값을 확인한 뒤 apply를 진행한다.

## terraform apply

plan 결과를 확인했으면 apply를 실행한다.
...
apply가 끝나면 Terraform outputs에서 주요 정보를 확인할 수 있다.
...
재사용 모드에서는 `openai_endpoint` 출력이 `null`임에 유의한다. 기존 Foundry 계정의 엔드포인트는 `existing_foundry_name`으로 이미 알고 있으므로 별도 출력이 없다.

## 다음 단계

게이트웨이가 정상적으로 배포됐는지 확인하려면 [05-verify/smoke-test.md](../05-verify/smoke-test.md)로 넘어간다.
