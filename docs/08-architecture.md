...
| `foundry`       | 신규 AIServices 계정/project와 지원 모델 deployment 생성, 또는 기존 계정/project 연결 |
...
| 계정 보안 설정 | Terraform이 관리 | 고객이 설정하고 Terraform이 검증 | 고객이 설정하고 Terraform이 검증 |

기존 계정 재사용 경로는 모델 deployment를 건드리지 않고 게이트웨이 연결과 거버넌스 레이어만 추가합니다. 자세한 절차는 [모델 백엔드 기존 계정 재사용](04-reuse-foundry.md)을 참고하세요.
...
* [모델 백엔드 기존 계정 재사용](04-reuse-foundry.md) — 기존 모델과 프로젝트 유무에 따른 연결 방식
