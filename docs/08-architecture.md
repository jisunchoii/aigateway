...
| `/responses`     | Codex CLI          | `Ocp-Apim-Subscription-Key` | Responses body-route via Codex proxy    |

path-route 요청은 APIM 정책에서 body-route로 정규화됩니다. 즉 URL의 deployment 이름을 body의 `"model"` 필드로 주입하고, backend에는 `/openai/v1/chat/completions` 형식으로 전달합니다. `/responses`만 예외로 Codex proxy sidecar가 응답/도구 payload를 정규화한 뒤 같은 `codexproj` project의 `/openai/v1/responses`를 호출합니다.
