...
<figure><img src="images/policy-pipeline.svg" alt="APIM 정책 파이프라인 — 요청 정규화, consumer 식별, 모델 권한, 예산 기반 모델 전환, 토큰 제한, 메트릭, 백엔드 인증"><figcaption>APIM 정책 파이프라인 — 동기 요청 처리와 설정·관측 평면의 연결</figcaption></figure>

### 클라이언트 엔드포인트

| APIM 경로 | 주 클라이언트 | 인증 헤더 | 요청 형식 |
|---|---|---|---|
| `/openai` | GitHub Copilot CLI | `api-key` | path-route (`/deployments/<model>/...`) |
| `/vscode/models` | VS Code BYOK | `Ocp-Apim-Subscription-Key` | path-route |
| `/foundry` | 직접 API 호출 | `Ocp-Apim-Subscription-Key` | body-route (`"model"` in body) |

path-route 요청은 APIM 정책에서 body-route로 정규화됩니다. 즉 URL의 deployment 이름을 body의 `"model"` 필드로 주입하고, backend에는 `/openai/v1/chat/completions` 형식으로 전달합니다.
...
| `openai` | greenfield Azure OpenAI 계정과 gpt 계열 deployment |
| `foundry` | greenfield AIServices 계정과 OSS/partner deployment, 또는 brownfield 계정 참조 |
| `apim` | APIM 인스턴스, API surfaces, policy XML, diagnostics |
| `config_store` | Cosmos DB 설정 저장소와 데이터 평면 RBAC |
| `registry` | ACR remote build 대상 |
| `control_plane` | config-sync worker와 Admin UI Container Apps |
| `jumpbox` | VNet 내부 seed/진단용 VM과 Bastion |

모듈 간 의존성은 아래처럼 흘러갑니다.

```text
network ─┬─ private endpoints ─▶ openai / foundry / cosmos / keyvault
         ├─▶ apim ─▶ backend RBAC + API policies
         └─▶ control_plane ─▶ worker + Admin UI

config_store ─▶ worker ─▶ APIM named values
observability ─▶ APIM diagnostics + Admin UI dashboards
registry ─▶ worker/admin-ui images
```
...
Cosmos DB의 `config` 컨테이너가 런타임 거버넌스의 원천입니다. config-sync worker가 이 문서를 읽어 APIM named values로 반영합니다.
