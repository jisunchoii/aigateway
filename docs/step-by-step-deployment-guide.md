...
- **게이트웨이 호출** (VNet 내부, 컨슈머 구독 키 사용):
  ...
  ```http
  POST https://<apim-host>/openai/deployments/<배포이름>/chat/completions
  POST https://<apim-host>/foundry/chat/completions      # 본문 "model"에 배포이름
  ```

  점프박스에서 `scripts/smoke-*.ps1`로 검증(스크립트가 있는 경우).
...
Claude Code / Codex CLI는 **OpenAI 호환 base URL**을 게이트웨이로 향하게 하고, 인증에는 Admin UI에서
발급한 **컨슈머 구독 키**를 사용합니다.

- **base URL**: `https://<apim-host>/foundry`(OpenAI v1 호환) 또는 `https://<apim-host>/openai`
  (Azure OpenAI 호환). External이면 공개 호스트, 옵션 2에서는 VNet 내부에서만 유효합니다.
- **인증 헤더**: `api-key: <키>` (이 게이트웨이의 구독 키 헤더는 Azure OpenAI 컨벤션을 따라
  `Ocp-Apim-Subscription-Key`가 아니라 **`api-key`**로 설정돼 있습니다. 쿼리 파라미터 `?api-key=`도 가능).
  그래서 Azure OpenAI용 클라이언트(`api-key` 헤더 사용)는 그대로 붙고, OpenAI용 도구는 키를
  `api-key` 헤더로 보내도록 맞추면 됩니다.

```powershell
$base = "https://<apim-host>"
$key  = "<consumer-subscription-key>"
curl "$base/foundry/chat/completions" `
