...
| 기존 계정, 모델, Foundry 프로젝트가 모두 있다 | [모델 백엔드 기존 계정 재사용](04-reuse-foundry.md#5-기존-프로젝트와-연결-리소스-import) |
...
{% hint style="info" %}
Codex CLI 등 Responses API 전용 클라이언트를 위한 `/openai/v1/responses` operation은 APIM 게이트웨이에 포함됩니다. `codexproxy_image`를 설정하면 Responses 요청을 Codex proxy sidecar가 정규화하며, `searchmcp_image`를 설정하면 OSS 모델용 `/mcp/` 웹 검색 경로가 활성화됩니다. [Codex CLI 온보딩](07-connect-clients/codex-cli.md) 참고.
{% endhint %}
...
| Image build | 애플리케이션 이미지를 ACR에 push | `config-sync-worker`, `admin-ui`, `codexproxy`, `searchmcp` 이미지 준비 |
...
`reuse_foundry=true`일 때 Terraform은 기존 모델 deployment를 관리하지 않습니다. 프로젝트가 없으면 새로 만들고, 프로젝트가 이미 있으면 `foundry_project_name`을 기존 이름과 맞춘 뒤 import합니다.
...
| 기존 모델 활용, 프로젝트 있음 | 기존 프로젝트 import, Private Endpoint·RBAC 생성 또는 import | 기존 계정·모델·프로젝트를 모두 유지하는 경우 |

기존 계정을 재사용하는 두 경우 모두 `reuse_foundry=true`를 사용합니다. Terraform은 계정과 모델 deployment를 삭제하거나 수정하지 않으며, 프로젝트 유무에 따라 새 프로젝트를 만들거나 기존 프로젝트를 import합니다.
