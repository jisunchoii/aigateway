...
{% hint style="info" %}
Codex CLI 등 Responses API 전용 클라이언트를 위한 `/responses` 경로는 APIM 게이트웨이 배포에 포함됩니다(별도 브리지 배포 불필요). AIServices 계정의 native Responses로 직접 프록시합니다. [Codex CLI 온보딩](07-connect-clients/codex-cli.md) 참고.
{% endhint %}
