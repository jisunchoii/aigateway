...
{% hint style="warning" %}
`apim_public`과 모델 백엔드 방식(`reuse_foundry`, `model_deployments`)은 첫 APIM 배포 전에 확정하세요. 나중에 바꾸면 APIM 정책, Private Endpoint, RBAC, 모델 라우팅을 다시 적용해야 합니다.
{% endhint %}
...
{% hint style="info" %}
Codex CLI 등 Responses API 전용 클라이언트를 위한 `/responses` 경로는 APIM 게이트웨이 배포에 포함됩니다. steady-state에서는 `/responses`만 Codex proxy sidecar를 거쳐 payload를 정규화하고, `/openai`·`/vscode/models`·`/foundry`는 같은 AIServices account/project를 직접 호출합니다. sidecar 이미지는 ACR이 생긴 뒤 `codexproxy_image`/`route_via_codexproxy`로 켭니다. [Codex CLI 온보딩](07-connect-clients/codex-cli.md) 참고.
{% endhint %}
...
| Verify | APIM 경유 모델 호출 검증 | `/openai`, `/vscode/models`, `/foundry`, `/responses` 정상 확인 |
...
| `model_deployments` | canonical 네 모델(`gpt-5.6-sol`, `FW-GLM-5.2`, `DeepSeek-V4-Pro`, `grok-4.3`) 또는 운영자가 승인한 동일 schema deployment map |
| `legacy_gpt_compat_enabled` | live migration GPT-family policy 호환. fresh/final은 `false` |
| `admin_ui_legacy_gpt_aliases_enabled` | live migration Admin UI legacy alias staging. fresh/final은 `false` |
| `rate_tiers` | consumer별 rate tier 정의 |
| `worker_image` | config-sync worker 활성화 여부 |
| `admin_ui_image` | Admin UI 활성화 여부 |

기존 state를 업그레이드할 때는 `reuse_foundry` 값을 이름만 보고 선택하지 말고 [기존 state 분류](04-reuse-foundry.md#기존-state-분류)를 먼저 수행합니다. Sidecar-era state가 managed project account를 이미 소유하면 `reuse_foundry=false`와 exact `foundry_account_name`을 사용합니다.
