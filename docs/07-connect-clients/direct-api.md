...
curl, REST Client, 애플리케이션 코드에서 APIM 게이트웨이를 직접 호출합니다. gpt 계열 모델은 `/openai` path-route를 사용하고, partner/OSS 모델은 `/foundry` body-route를 사용합니다.
...
```bash
curl -s -X POST "https://<apim-host>/openai/deployments/gpt-5.4/chat/completions?api-version=2025-01-01-preview" \
