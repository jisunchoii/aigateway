...
OpenAI Codex CLI를 custom model provider로 설정해 APIM 게이트웨이를 통과하도록 구성합니다. Codex CLI는 **Responses API 전용**(`wire_api = "responses"`)입니다. 게이트웨이의 `/responses` 경로는 AIServices(Foundry) 계정의 native Responses 엔드포인트(`/openai/v1/responses`)로 **직접** 프록시합니다.

{% hint style="warning" %}
**이 가이드는 gpt 계열(Azure OpenAI) 모델 전용입니다.** Codex CLI는 OpenAI 자사 모델(gpt/o-series)에 최적화돼 있어, grok·DeepSeek 같은 비gpt reasoning 모델은 native Responses를 지원하더라도 Codex에서 간헐적 mid-session 스트리밍 실패가 발생합니다(Codex의 알려진 한계 — [openai/codex#28742](https://github.com/openai/codex/issues/28742), [#16397](https://github.com/openai/codex/issues/16397) 등, 게이트웨이·backend 문제 아님). **grok·DeepSeek을 코딩 에이전트로 쓰려면 [OpenCode](opencode.md)**(Chat Completions 기반, `/foundry` 경로)**를 사용하세요.**
{% endhint %}

{% hint style="info" %}
`/responses`는 `/foundry`·`/openai`와 같은 APIM API로, 동일한 거버넌스 정책과 관리 ID(Managed Identity) 인증을 사용합니다. 별도 브리지 컨테이너가 없으므로 APIM만 배포되어 있으면 바로 쓸 수 있습니다.
{% endhint %}
...
* OpenAI Codex CLI를 **gpt 계열 모델**로 사용한다.
...
| 항목       | 값                                                      |
| -------- | ------------------------------------------------------ |
| base URL | `/responses` (APIM → AIServices native)                |
| 요청 경로    | Codex가 `base_url` + `/responses` 호출 → APIM이 `/openai/v1/responses`로 프록시 |
...
| 모델              | Codex 지원 | 비고                                        |
| --------------- | --------- | ----------------------------------------- |
| `gpt-5.4`       | ✅ 권장     | native Responses, Codex와 안정적 동작           |
...
{% hint style="info" %}
Fireworks 서빙 등 **partner/community 카테고리 OSS 모델**(GLM·Kimi 등)은 배포 capability상 Chat Completions만 노출하지만, Foundry **project 경로**(`/api/projects/<project>/openai/v1/responses`)로는 native Responses가 동작합니다(실측 200). 다만 Codex가 보내는 payload를 backend가 받는 형태로 변환해야 하며, 이는 정식 게이트웨이 경로가 아니라 **실험적 로컬 프록시**로만 검증됐습니다 → [부록: Fireworks 모델 실험 경로](#부록-fireworks-모델-실험-경로). 정식 지원이 필요하면 [OpenCode](opencode.md)의 Chat Completions 경로를 사용하세요.
{% endhint %}
...
```bash
codex -m gpt-5.4 "Reply with the single word: pong"
```

Codex TUI 안에서는 `/model` 슬래시 명령으로도 gpt 계열 모델을 전환할 수 있습니다. Admin UI에서 해당 consumer의 allowed models에 선택한 모델이 포함되어 있어야 합니다.

<figure><img src="../.gitbook/assets/codex-cli-responses.png" alt="Codex CLI에서 APIM /responses 경유로 gpt-5.4 응답을 받는 터미널 화면"><figcaption><p>Codex CLI 실행 결과 — APIM <code>/responses</code> 경유 native Responses 응답 (실제 터미널 스크린샷으로 교체하세요)</p></figcaption></figure>
...
* [Microsoft Foundry — Responses API로 텍스트 생성](https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-models/how-to/generate-responses)
...
## 부록: Fireworks 모델 실험 경로

{% hint style="danger" %}
**실험적 · 로컬 전용 · APIM 거버넌스 우회.** 아래 프록시는 정식 온보딩 경로가 아닙니다. Codex를 Fireworks 서빙 모델(GLM·DeepSeek·Kimi)로 구동하려는 개발 검증용이며, consumerId·allowed-models·rate limit·budget downgrade 등 게이트웨이 거버넌스가 **적용되지 않습니다**. 프로덕션·다중 사용자 용도가 아닙니다.
{% endhint %}

Codex는 Responses 전용이면서 OpenAI 고유의 툴 shape(`local_shell`, `custom`=apply_patch, `namespace`=multi_agent_v1)와 필드(`text.verbosity`, `include: reasoning.encrypted_content`)를 보냅니다. Foundry의 OpenAI 호환 계층은 이를 거부하므로, 로컬 번역 프록시가 backend가 받는 형태로 변환합니다. 스크립트: `scripts/foundry_codex_proxy.py` (stdlib만, 의존성 없음).
...
| `reasoning.effort` | GLM은 유지, DeepSeek·Kimi는 제거 | 모델별 허용 형태가 다름 |
...
python scripts/foundry_codex_proxy.py   # 127.0.0.1:8789

# 2) Codex 설정: model_providers.foundryproxy (base_url = http://127.0.0.1:8789/v1, wire_api = responses)
#    프로필 glm/deepseek/kimi.config.toml — model, model_provider="foundryproxy",
...
* Kimi는 배포 quota(429)에 걸릴 수 있습니다.
