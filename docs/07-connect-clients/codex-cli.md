...
OpenAI Codex CLI를 custom model provider로 설정해 APIM 게이트웨이를 통과하도록 구성합니다. Codex CLI는 **Responses API 전용**(`wire_api = "responses"`)입니다. 게이트웨이의 `/responses` 경로는 Codex proxy sidecar가 payload를 정규화한 뒤 같은 AIServices 계정 아래 `codexproj` project의 `/api/projects/codexproj/openai/v1/responses`로 전달합니다.

{% hint style="warning" %}
**이 가이드는 `gpt-5.6-sol` 전용입니다.** Codex CLI는 OpenAI 자사 모델(gpt/o-series)에 최적화돼 있어, grok·DeepSeek 같은 `gpt-5.6-sol` 이외 reasoning 모델은 native Responses를 지원하더라도 Codex에서 간헐적 mid-session 스트리밍 실패가 발생합니다(Codex의 알려진 한계 — [openai/codex#28742](https://github.com/openai/codex/issues/28742), [#16397](https://github.com/openai/codex/issues/16397) 등, 게이트웨이·backend 문제 아님). **grok·DeepSeek을 코딩 에이전트로 쓰려면 [OpenCode](opencode.md)**(Chat Completions 기반, `/foundry` 경로)**를 사용하세요.**
{% endhint %}
...
**맞지 않는 경우**: grok·DeepSeek 등 `gpt-5.6-sol` 이외 모델을 쓰려면 [OpenCode](opencode.md)를 사용하세요.
{% endhint %}
...
{% hint style="info" %}
`wire_api`는 `"responses"`가 유일한 값입니다(Codex CLI는 chat completions wire 포맷을 더 이상 지원하지 않습니다). 현재 정식 게이트웨이 경로에서는 `gpt-5.6-sol`만 native Responses를 완전 지원하는 Codex 대상 모델로 문서화합니다.
{% endhint %}
...
| 요청 경로    | Codex가 `base_url` + `/responses` 호출 → APIM이 Codex proxy로 전달 → proxy가 `/api/projects/codexproj/openai/v1/responses` 호출 |
...
Codex가 보내는 요청은 항상 Responses 포맷이며, 현재 정식 backend는 `gpt-5.6-sol` 하나이고 이 deployment가 native Responses를 지원하므로 hosted tool(`web_search`)과 function tool을 포함해 그대로 전달됩니다.
...
* **거버넌스 없음** (위 danger 참고).
* Codex↔`gpt-5.6-sol` 이외 reasoning 모델의 간헐적 mid-session 스트림 중단은 프록시로 해결되지 않습니다(Codex 클라이언트 한계).
