...
OpenAI Codex CLI를 custom model provider로 설정해 APIM 게이트웨이를 통과하도록 구성합니다. Codex CLI는 **Responses API 전용**(`wire_api = "responses"`)이며, 게이트웨이의 `/openai/v1/responses` 경로는 Codex proxy sidecar가 모델별 payload를 정규화한 뒤 AIServices project로 전달합니다.

{% hint style="warning" %}
`gpt-5.6-sol`은 native hosted tool을 사용할 수 있습니다. `FW-GLM-5.2`, `DeepSeek-V4-Pro`, `grok-4.3` 요청에서는 불안정한 server-hosted tool을 제거하므로, 웹 검색은 이 가이드의 Search MCP를 사용합니다.
{% endhint %}
...
| -------- | --- |
| base URL | `/openai/v1` |
| 요청 경로    | Codex가 `base_url` + `/responses` 호출 → APIM → Codex proxy → AIServices project |
| APIM 인증  | `api-key` 헤더 |
...
| 모델 | Codex 지원 | 처리 |
|---|---|---|
| `gpt-5.6-sol` | ✅ 권장 | native Responses와 hosted tool 전달 |
