...
OpenCode에서 APIM 게이트웨이를 통해 gpt 계열과 partner/OSS 모델을 사용하도록 설정합니다. APIM 경유 시에는 OpenCode의 **OpenAI-compatible custom provider**를 사용합니다.
...
| gpt 계열 APIM base URL | `https://<apim-host>/openai/deployments/gpt-5.4` |
...
```json
{
  ...
  "small_model": "aigateway-openai/gpt-5.4",
  "provider": {
    "aigateway-openai": {
      "npm": "@ai-sdk/openai-compatible",
      "name": "AI Gateway APIM OpenAI",
      "options": {
        "baseURL": "https://<apim-host>/openai/deployments/gpt-5.4",
        "queryParams": {
          "api-version": "2025-01-01-preview"
        },
        "headers": {
...
|---|---|---|
| OpenCode model ID | `aigateway-openai/gpt-5.4` | `aigateway/DeepSeek-V4-Pro`, `aigateway/grok-4.3` |
...
```bash
opencode --model aigateway-openai/gpt-5.4
```

OpenCode TUI 안에서는 `/model` 명령으로도 전환할 수 있습니다. 모델 목록에서 `aigateway-openai/gpt-5.4`, `aigateway/DeepSeek-V4-Pro`, `aigateway/grok-4.3` 중 하나를 선택합니다.

<figure><img src="../images/opencode-model-picker.png" alt="OpenCode TUI에서 /model 명령으로 APIM provider 모델을 선택하는 화면"><figcaption>OpenCode `/model` 모델 선택 — APIM 경유 provider 확인</figcaption></figure>
...
partner/OSS 경로도 스크린샷에 포함하려면 `/model`로 `aigateway/DeepSeek-V4-Pro` 또는 `aigateway/grok-4.3`로 전환한 뒤 같은 프롬프트를 한 번 더 실행합니다.

| 멀티에이전트 검증 | 서브에이전트 검증 |
|---|---|
| <img src="../images/opencode-multi-agent.png" alt="OpenCode TUI에서 APIM 경유 모델로 멀티에이전트 검증을 수행하는 화면"> | <img src="../images/opencode-sub-agent.png" alt="OpenCode TUI에서 APIM 경유 모델로 서브에이전트 검증을 수행하는 화면"> |
