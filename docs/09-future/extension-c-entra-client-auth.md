...
`client_auth_mode="entra-id"`로 설정하면 APIM inbound 정책이 아래와 같이 바뀐다.

1. `validate-jwt` 정책이 `Authorization: Bearer <token>` 헤더를 검증한다
2. JWT의 지정된 claim 값을 `consumerId`로 추출한다
3. 이후 allowed-models 검사, rate limit, 예산 모델 전환 등 모든 거버넌스 파이프라인은 동일하게 동작한다

구독 키 검증이 없으므로 `subscription_required=false`로 설정해야 한다.
...
`client_auth_mode="entra-id"` 모드에서 consumerId를 어떤 JWT claim에서 가져올지는 중요한 설계 결정이다.
...
consumerId로 사용할 단일 값을 명시적으로 표현하려면 아래 방법 중 하나를 권장한다.
...
두 방법 모두 단일 값으로 consumerId를 명확히 표현할 수 있고, groups over-claim 문제를 피할 수 있다.
...
`team_claim` 변수가 consumerId를 추출할 JWT claim 이름을 지정한다.
