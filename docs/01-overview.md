...
* `/openai/v1/chat/completions`는 모든 지원 모델의 통합 Chat Completions 경로이고, `/openai/v1/responses`는 필요한 경우 Codex proxy가 payload를 정규화한 뒤 같은 프로젝트로 전달합니다.
* VS Code BYOK는 `/vscode/models/deployments/{model}/chat/completions`, Search MCP는 `/mcp/` 전용 경로를 사용합니다.
...
클라이언트의 AI 요청은 APIM을 거쳐 거버넌스 정책을 적용받고, Private Endpoint로 연결된 기준 AIServices 계정과 프로젝트로 전달됩니다. 응답 이후에는 토큰 메트릭이 기록되고, config-sync worker가 비동기로 예산 상태를 평가해 다음 요청의 모델 전환 여부를 결정합니다. Admin UI의 모델 picker는 Terraform이 `model_deployments`에서 생성한 `ALIAS_MODELS_JSON`을 사용하고, config-sync는 Cosmos 기반 APIM runtime named value만 갱신합니다.
