...
- 백엔드는 **Microsoft Foundry (AIServices)** 단일 계정입니다. 
- gpt-5.4 계열은 `azurerm_cognitive_account`(`/openai/v1` 경로)로, grok-4.3·DeepSeek-V4-Pro 등 OSS/파트너 모델은 같은 계정의 Foundry 배포로 제공합니다. 
- 모든 백엔드 호출은 동일한 `/openai/v1/chat/completions` 형식을 사용하며, 클라이언트 요청 형식(path-route vs body-route)은 APIM 정책이 변환합니다.
...
클라이언트의 AI 요청은 APIM을 거쳐 거버넌스 정책을 적용받고, Private Endpoint로 연결된 백엔드 모델로 전달됩니다. 응답 이후에는 토큰 메트릭이 기록되고, config-sync worker가 비동기로 예산 상태를 평가해 다음 요청의 모델 전환 여부를 결정합니다.

<figure><img src="images/request-e2e-flow.svg" alt="요청 흐름 — 개발 툴에서 APIM 정책, 백엔드 호출, 메트릭 기록, 예산 평가까지 이어지는 end-to-end 흐름"><figcaption>요청 end-to-end 흐름 — APIM 정책 처리, 백엔드 호출, 메트릭 기록, 비동기 예산 평가</figcaption></figure>
