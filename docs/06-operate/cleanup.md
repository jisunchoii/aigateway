> 읽는 사람: 게이트웨이 운영자·인프라 담당자 · 선행: 06-operate/scale-sku.md
...
> `<rg>`는 `terraform output -raw resource_group_name` 으로 확인합니다.
...
> 역할 할당과 Private Endpoint가 제거되므로 Foundry 계정의 네트워크·접근 설정이 변경됩니다. 기존 Foundry 계정을 다른 서비스에서도 사용 중이라면 영향을 미칠 수 있으니 사전 확인이 필요합니다.
