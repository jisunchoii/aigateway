...
```bash
az resource show \
  --ids '<existing-aiservices-account-resource-id>/projects/<existing-project-name>' \
  --query id -o tsv
```

출력된 project parent가 재사용할 AIServices account ID와 일치하는지 확인합니다. 이 경로의 plan에는 `module.foundry.azapi_resource.project[0]`이 없어야 하며, 이후 `terraform destroy`도 기존 프로젝트를 삭제하지 않습니다.
