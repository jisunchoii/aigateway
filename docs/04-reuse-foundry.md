...
<figure><img src="images/diagram-reuse-topology.svg" alt="재사용 토폴로지 — 기존 계정과 모델은 유지하고 Private Endpoint와 RBAC만 추가"><figcaption>기존 계정과 모델 배포는 Terraform 소유 리소스로 편입하지 않고, gateway 쪽 연결 리소스만 추가합니다.</figcaption></figure>
...
| 키 인증·공용 접근 차단 | Terraform이 설정 | 배포 전 고객이 `az` CLI로 설정 |
| Private Endpoint | 생성 | gateway VNet에 새로 생성 |
| APIM MI RBAC | 부여 | 기존 계정 scope에 부여 |

재사용 모드(`reuse_foundry=true`)에서는 `modules/openai`가 비활성화되고, gpt 계열 요청도 기존 AIServices 계정의 `/openai/v1` 경로로 라우팅됩니다.

## 3. 배포 전 기존 계정 보안 설정

gateway는 APIM managed identity와 RBAC로 backend를 호출합니다. 따라서 Terraform apply 전에 기존 계정의 **API key 인증**과 **공용 네트워크 접근**을 꺼서, gateway가 아닌 직접 호출 경로를 차단해야 합니다.
...
allowed_models = ["grok-4.3", "DeepSeek-V4-Pro"]
```
...
| `foundry_deployments` | 기존 계정에 이미 존재하는 모델 배포 선언 |
| `allowed_models` | APIM 정책과 Admin UI에서 허용할 모델 목록 |

{% hint style="danger" %}
`foundry_deployments`의 **map key는 실제 배포 이름과 대소문자까지 정확히 일치**해야 합니다. 이 값이 allowed model 검사, backend 라우팅, Admin UI 표시 이름에 함께 사용됩니다.
{% endhint %}

재사용 모드에서는 `openai_deployments`가 적용되지 않습니다. 별도 Azure OpenAI 계정을 만들지 않고, gpt 계열도 기존 AIServices 계정의 deployment 이름을 `foundry_deployments`에 선언해 사용합니다.
...
{% hint style="warning" %}
plan에 `destroy` 또는 기존 AIServices 계정 변경이 보이면 apply하지 말고 `reuse_foundry`, `existing_foundry_name`, `existing_foundry_rg`, `foundry_deployments` 값을 다시 확인하세요.
{% endhint %}
